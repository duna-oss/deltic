...
type LastTypeOf<T extends readonly unknown[]> = T extends readonly [...unknown[], infer Last]
    ? Last
    : never;

type TupleWithoutFirst<T extends [any, ...any[]]> = T extends [any, ...infer Rest]
    ? Rest
    : []

interface TransformFunction<Type, AggregateRootId extends string | number, From, To> {
    (
        message: Message<Type, From, AggregateRootId>,
    ): Message<Type, To, AggregateRootId>;
}
...
type OnlyMessagesThatNeedUpcasting<M extends VersionedMessageDefinition> = {
    [T in keyof M as M[T] extends [any, any, ...any[]]
        ? T
        : never]: M[T] extends [any, any, ...any[]]
        ? M[T]
        : never
}

type UpcastersForMessages<
    AggregateRootId extends string | number,
    Messages extends MessagesThatNeedUpcasting,
> = {
    [T in keyof Messages]: UpcasterFuncs<T, AggregateRootId, TupleWithoutFirst<Messages[T]>, Messages[T]>
};

type UpcastersForVersionedStream<Stream extends VersionedStreamDefinition<Stream>>
    = UpcastersForMessages<Stream['aggregateRootId'], Stream['upcasters']>

export type DefineVersionedStream<Stream extends VersionedStreamStructure> =
    Omit<Stream, 'messages'> & {
    messages: {
...
export class SchemaVersionMessageDecorator<Stream extends VersionedStreamDefinition<Stream>> implements MessageDecorator<Stream> {
    constructor(
...
export class VersionedMessageConsumer<Stream extends VersionedStreamDefinition<Stream>> implements MessageConsumer<Stream> {
    constructor(
        private readonly upcasters: UpcastersForVersionedStream<Stream>,
...
export class UpcastingMessageRepository<Stream extends VersionedStreamDefinition<Stream>> implements MessageRepository<Stream> {
    constructor(
        private readonly upcasters: UpcastersForVersionedStream<Stream>,
    ...
    retrieveAllAfterVersion(id: Stream['aggregateRootId'], version: number): AsyncGenerator<AnyMessageFrom<Stream>> {
        return this.upcastMessages(
            this.repository.retrieveAllAfterVersion(id, version),
        );
    }

    retrieveAllForAggregate(id: Stream['aggregateRootId']): AsyncGenerator<AnyMessageFrom<Stream>> {
        return this.upcastMessages(
            this.repository.retrieveAllForAggregate(id),
        );
    }

    retrieveAllUntilVersion(id: Stream['aggregateRootId'], version: number): AsyncGenerator<AnyMessageFrom<Stream>> {
        return this.upcastMessages(
            this.repository.retrieveAllUntilVersion(id, version),
        );
    }

    retrieveBetweenVersions(id: Stream['aggregateRootId'], after: number, before: number): AsyncGenerator<AnyMessageFrom<Stream>> {
        return this.upcastMessages(
            this.repository.retrieveBetweenVersions(id, after, before),
        );
    }

    private async* upcastMessages(messages: AsyncGenerator<AnyMessageFrom<Stream>>): AsyncGenerator<AnyMessageFrom<Stream>> {
...
export class UpcastingOutboxRepository<Stream extends VersionedStreamDefinition<Stream>> implements OutboxRepository<Stream> {
    constructor(
        private readonly upcasters: UpcastersForVersionedStream<Stream>,
