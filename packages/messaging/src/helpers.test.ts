...
describe('Messaging helper functions', () => {
    test('createMessageDispatcher creates a producer from a function', async () => {
        ...
        const producer = createMessageDispatcher(actualProducer.send.bind(actualProducer) as typeof actualProducer.send);
        const message: AnyMessageFrom<ExampleStream> = {payload: 'lol', headers: {}, type: 'example'};
