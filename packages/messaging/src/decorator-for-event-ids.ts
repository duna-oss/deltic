...
export class MessageDecoratorForEventIds<
    Stream extends StreamDefinition = StreamDefinition,
> implements MessageDecorator<Stream> {
    constructor(private readonly idGenerator: () => number | string) {}

    decorate(messages: MessagesFrom<Stream>): MessagesFrom<Stream> {
        return messages.map(message =>
            message.headers['event_id'] === undefined
