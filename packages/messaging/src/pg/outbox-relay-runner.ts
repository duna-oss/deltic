...
export class OutboxRelayRunner<Stream extends StreamDefinition> {
    ...
    private async processBatch(): Promise<void> {
        if (!await this.singleProcessingMutex.tryLock()) {
            /**
             * A batch is already being processed. This happens when multiple
             * notifications arrive in rapid succession. Returning early is safe
             * because the in-flight batch will continue processing if messages
             * remain, making it safe to ignore the notification.
             */
            return;
        }
